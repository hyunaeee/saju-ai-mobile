...
(function () {
  ...
  const api = {
    ...
    mount(hostSel, o) {
      ...
      host.innerHTML = `
        <div class="bi">
          ...
          <div class="bi-row bi-time">
            <select id="${id}-h" aria-label="태어난 시">${opts(0, 23, v => String(v).padStart(2, "0") + "시", defH)}</select>
            <select id="${id}-min" aria-label="태어난 분">${
              Array.from({ length: 12 }, (_, i) => i * 5).map(v =>
                `<option value="${v}"${v === defMin - (defMin % 5) ? " selected" : ""}>${String(v).padStart(2, "0")}분</option>`).join("")}</select>
            <select id="${id}-r" aria-label="태어난 지역">${REGIONS.map((r, i) =>
              `<option value="${i}"${i === defR ? " selected" : ""}>${r.name}</option>`).join("")}</select>
          </div>
          <label class="bi-unknown">
            <input type="checkbox" id="${id}-unknown"${unknown ? " checked" : ""} /> <span>태어난 시각을 몰라요 (시주 없이 봅니다)</span>
          </label>
          <div class="bi-row bi-seg2">
          ...
          <p class="bi-note">태어난 지역의 경도로 <b>진태양시</b>를 보정하고, 음력은 한국천문연구원 기준으로 <b>양력 자동 변환</b>합니다. 입력은 이 기기에만 저장됩니다.</p>
        </div>`;
      ...
      const timeSel = [$(`#${id}-h`), $(`#${id}-min`)];
      const toggleUnknown = () => {
        const un = $(`#${id}-unknown`).checked;
        timeSel.forEach(s => { s.disabled = un; s.style.opacity = un ? ".45" : ""; });
      };
      $(`#${id}-unknown`).addEventListener("change", toggleUnknown);
      toggleUnknown();
    ...
    read(host, id) {
      ...
      return {
        y, m, d, lunar,
        hour: un ? -1 : +$(`#${id}-h`).value,
        minute: un ? 0 : +$(`#${id}-min`).value,
    ...
    metaLine(input, saju) {
      ...
      const region = (typeof REGIONS !== "undefined" && REGIONS[input.regionIdx]) ? REGIONS[input.regionIdx].name : "";
  ...
  .bi-time { grid-template-columns:1fr 1fr 1.25fr; }
